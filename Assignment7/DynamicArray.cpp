...
//Constructors
//Default
DynamicArray::DynamicArray() : capacity(DEFAULTCAPACITY), numOfItems(0) { data = new unsigned int[capacity]; }
...
unsigned int& DynamicArray::operator [] (size_t index) {
	return data[index];
}

//Subscript operator (rvalue)
const unsigned int& DynamicArray::operator [] (size_t index) const {
	return data[index];
}
...
*/
void DynamicArray::pushBack(unsigned int value) {
	if (size() == capacity) {
		resize();
	}
